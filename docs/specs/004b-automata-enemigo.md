...
hacer daño real al jugador y provocar Derrota. Sus personajes tienen la vida multiplicada (trampa) y
disponen de un reroll extra de blancos más allá del gratuito.

## Criterios de aceptación

Verificables jugando. Formato: acción → resultado observable.

- [ ] Tras importar el mazo enemigo, sus personajes muestran la vida **ya multiplicada** (p. ej. si
      el multiplicador es x1.5 y la vida base es 10, se ve 15), tanto en vida máxima como en la
      vida inicial disponible.
- [ ] Con el pool enemigo vacío y personajes enemigos sin activar → pulsar "Turno enemigo" **activa**
      el personaje enemigo de más vida restante y tira sus dados a su pool (visible en pantalla).
...
- [ ] Con el reroll gratuito ya gastado esta ronda y 2+ blancos aún en el pool → pulsar "Turno
      enemigo" gasta el **reroll extra de la trampa** (una vez más por ronda) si está disponible.
...
- **Empate de "menor vida restante" entre varios personajes del jugador** → desempate determinista
  (por orden de aparición/índice de instancia), documentado en Notas técnicas, no aleatorio.
...
- **Multiplicador de vida sobre valores no enteros** (p. ej. x1.5 sobre vida impar) → redondear de
  forma consistente (definir en Notas técnicas al implementar, documentando el redondeo elegido).
...
- **Trampas como constantes de configuración** (no UI): p. ej. `ENEMY_HEALTH_MULTIPLIER = 1.5`,
  `ENEMY_EXTRA_REROLLS_PER_ROUND = 1`, en un único sitio fácil de tocar más adelante cuando exista
  selector de dificultad.
- **Vida multiplicada**: aplicar el multiplicador a la vida base del personaje enemigo en el
  momento de importar el mazo (o al calcular vida máxima mostrada), coherente con cómo SPEC-003
  calcula `character.health - damage[i]`.
...
- **Desempate de objetivo**: al elegir "personaje del jugador con menos vida restante", si hay
  empate usar el mismo criterio de índice de instancia que SPEC-002/003 (orden estable, no
  aleatorio) para que el comportamiento sea determinista y testeable.
...
solo falta que se dispare jugando). Riesgo de superar ~300 líneas por la combinación motor+tests+UI.
Al empezar a implementar, valorar dividir en: **(004b-1)** motor del autómata puro + tests (sin UI)
+ trampa de vida multiplicada; **(004b-2)** botón "Turno enemigo" en la UI + contador de rerolls por
ronda + enganche de Derrota jugable.
