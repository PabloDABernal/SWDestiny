...
- Tabla `SET_CODES` de `parseTextDeck` incompleta (SPEC-017/023): tras "Transformations" (código 13, el último set oficial FFG), ARH DB tiene una continuación fan "A Renewed Hope" con 11 sets propios (desplegable "Sets" de la web: Faltering Allegiances, Redemption, High Stakes, Echoes of Destiny, Unlikely Heroes, Galactic Struggle, Seeking Answers, Display of Power, Resurgence, Awaiting Fate, Uncharted Alliances). Confirmados hasta ahora: High Stakes=16, Display of Power=22, Awaiting Fate=24. **El orden del desplegable NO se corresponde con la numeración interna real** (High Stakes es la 3ª de la lista pero código 16, no 03; Display of Power la 8ª pero 22; Awaiting Fate la 10ª pero 24) — no se puede deducir el código de los 8 sets restantes por su posición en ese menú, hay que confirmarlos uno a uno igual que los tres anteriores (abrir una carta del set en db.swdrenewedhope.com y mirar el prefijo numérico de la URL). Cualquier mazo con un set no confirmado fallará el import con "No reconozco el set...". (Detectado en SPEC-023, 2026-07-23.)
