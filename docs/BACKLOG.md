...
- Tabla `SET_CODES` de `parseTextDeck` incompleta (SPEC-017/023): el salto de "Transformations" (código 13) a "Display of Power" (22) y "Awaiting Fate" (24, ambos confirmados jugando SPEC-023) sugiere que ARH ha publicado más sets intermedios (14-21, 23) todavía no confirmados/añadidos a la tabla. Cualquier mazo con esos sets fallará el import con "No reconozco el set..." hasta que el usuario confirme su código real (mismo proceso: abrir una carta del set en db.swdrenewedhope.com y mirar el prefijo numérico de la URL). (Detectado en SPEC-023, 2026-07-23.)
