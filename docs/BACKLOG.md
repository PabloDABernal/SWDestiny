...
- Permitir combinar varios dados base de daño indirecto propio del jugador en un mismo reparto (hoy SPEC-026 exige resolver uno a uno, marcar un segundo mientras el primero está pendiente es no-op): el usuario propone revisar esa regla para que indirecto se agrupe como melee/ranged. Requiere decidir de nuevo la regla de juego (contradice SPEC-026 tal como está escrita) antes de tocar código. (Detectado jugando SPEC-039, 2026-07-27.)
- El aviso de "última acción" del jugador (`hint` en `src/App.tsx`) trata el símbolo **indirecto** (SPEC-026) igual que melee/ranged ("Pulsa un personaje enemigo para aplicarlo"), aunque indirecto ya se resuelve con su propio botón sin objetivo desde SPEC-026 — mensaje engañoso preexistente, detectado al añadir el mismo tipo de excepción para disrupt/descarte en SPEC-029. No se corrige aquí (fuera del alcance de esta spec); arreglar cuando se retoque ese hint.
