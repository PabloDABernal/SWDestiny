...
- Daño indirecto (◎) real, segunda pieza — "el enemigo ataca con indirecto, el jugador reparte": SPEC-026 (2026-07-24) solo cubre la dirección "el jugador ataca, el autómata reparte solo". Esta pieza (candidata a SPEC-027) es la contraria: cuando el autómata ataca con un dado indirecto, hoy (y hasta que exista esa spec) lo sigue tratando igual que melee/ranged (él mismo elige el objetivo) — simplificación explícita de v1 que sigue vigente para esta dirección. Requiere: (a) separar el símbolo indirecto del resto de "daño" en `isDamageSymbol`/`combineAutomatonBatch` (`src/game/automaton.ts`), que hoy los agrupa; (b) un sub-modo interactivo nuevo para que el jugador (defensor) reparta clic a clic sobre sus propios personajes, con el guard de turno invertido (el defensor puede clicar aunque no sea su turno, precedente de Reroll de dado SPEC-023, pero el guard exacto hay que fijarlo, no es un calco directo — señalado por revisor-specs revisando SPEC-026). Decisión ya tomada por el usuario (2026-07-24): cuando el autómata ataca con varios dados indirecto en la misma tanda, SÍ puede combinarlos en un único total (a diferencia del jugador, que resuelve uno a uno) — no perder esta decisión al escribir esa spec.
